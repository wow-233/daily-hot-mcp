...
class DeepSeekTranslator:
    ...
    def __init__(self):
        ...
        self.proxy = Config.get_proxy_dict()
        
        if not self.api_key:
            raise ValueError("API密钥未设置，请在 .env 中配置 API_KEY")
    ...
    def translate_text(self, text: str, target_language: str = "中文") -> str:
        ...
        try:
            prompt = f"""请将以下英文文本翻译成中文，保持专业性和准确性，只翻译原文本身：
...
请直接给出中文总结，不要添加任何解释或前缀："""
    ...
    def translate_chinese_news(self, news_list: List[Dict]) -> List[Dict]:
        """
        翻译中文热门新闻（无需翻译，只需要保持格式）
        ...
        for news in news_list:
            ...
            news['summary_zh'] = f"热度: {news.get('hot', '')}"
            translated_news.append(news)
        
        logger.info(f"处理了 {len(translated_news)} 条中文新闻")
        return translated_news
